{
  ...
  nix.settings =
    ...
    # Conditionally enable lazy-trees for Determinate Nix
    // (
      if repoConfig.useDeterminateNix
      then {lazy-trees = true;} # Enable lazy trees for faster evaluations and reduced disk usage
      else {}
    )
    // {
      ...
      substituters = [
        ...
        "https://felixschausberger.cachix.org?priority=3"
