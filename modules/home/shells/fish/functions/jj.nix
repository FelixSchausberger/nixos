{
  ...
  # ocws - opencode workspace. Creates a jj workspace for a single agent,
  # claims it with a change description, and starts opencode rooted there.
  # jj workspaces are the shared-repo equivalent of git worktrees and are the
  ...
  # generated by the opencode v2 module.
  ocwsCmd = pkgs.writeShellApplication {
    name = "ocws";
    runtimeInputs = [
      pkgs.jujutsu
      pkgs.coreutils
    ...
    text = ''
      ...
      root="$(jj workspace root 2>/dev/null)" || {
        echo "ocws: not inside a jj workspace" >&2
        exit 1
      }
      ...
      # Optional leading flag: run the opencode 2 beta instead of V1.
      if [ "$sub" = "--v2" ]; then
        OCWS_V2=1
        shift
        sub="''${1:-}"
      ...
      shift || true
      [ "''${1:-}" = "--" ] && shift
      valid_task "$task" || { echo "ocws: invalid task '$task' (use [A-Za-z0-9._-])" >&2; exit 2; }

      dir="$(ws_dir_for "$task")"
      if workspace_exists "$task"; then
        echo "ocws: workspace '$task' already exists - 'ocws rm $task' to recreate it" >&2
        exit 1
      fi
      ...
      if [ "$new_tab" = "1" ]; then
        if [ -z "''${ZELLIJ_SESSION_NAME:-}" ]; then
          echo "ocws: 'tab' must run inside a zellij session" >&2
          exit 1
        fi
        zellij action new-tab --name "$task" --cwd "$dir" -- "''${run[@]}"
      else
        cd "$dir"
        exec "''${run[@]}"
      fi
...
in {
  home.packages = [jjworkCmd jjpushCmd jjtidyCmd jjtestCmd ocwsCmd];
