...
in {
  ...
  config = lib.mkMerge [
    {
      # Provide default empty actions table when system niri module not imported
      # This gets overridden by the real actions library from niri-flake when available
      lib.niri.actions = lib.mkDefault {};
    }
    (lib.mkIf (cfg.enable && (config.lib.niri.actions ? spawn)) {
      home = {
        packages = with pkgs; [
      ...
      programs.niri.settings = {
        ...
        binds = with config.lib.niri.actions; {
          ...
          # Notifications
          "Mod+Escape".action = spawn "${pkgs.libnotify}/bin/notify-send" "Test" "Wired notification system";
