...
get_nix_env_info() {
	...
	[[ -f "flake.nix" ]] && printf "\033[1;94mflake\033[0m" && return

	# Check active nix environments
	[[ "$IN_NIX_SHELL" == "pure" ]] && printf "\033[1;94mnix-pure\033[0m" && return
	[[ -n "$IN_NIX_SHELL" ]] && printf "\033[1;94mnix-shell\033[0m" && return
	[[ -n "$DEVSHELL_ROOT" ]] && printf "\033[1;94mdev-shell\033[0m" && return

	# Check for nix files
	[[ -f "shell.nix" || -f "default.nix" ]] && printf "\033[1;94mnix\033[0m" && return
}
...
get_context_usage() {
	...
	local context_size
	context_size=$(echo "$input" | jq -r '.context_window.context_window_size // 0' 2>/dev/null || echo "0")
	local usage
	...
	if [[ $percent_used -lt 10 ]]; then
		...
		printf "%b\n" "${color}${whole}.${decimal}%\033[0m"
	else
		printf "%b\n" "${color}${percent_used}%\033[0m"
	fi
}
