# Defines the hostConfig NixOS module option shared across all hosts.
# All system and home modules receive hostConfig and self-guard with lib.mkIf
# rather than relying on top-level enable flags.
{
  lib,
  config,
  inputs,
...
in {
  ...
  in {
    ...
    system.activationScripts.checkConfigLocation =
      lib.mkIf (lib.hasAttr "persistence" config.environment)
      {
        text = ''
